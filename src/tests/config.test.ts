...
test('CONFIG - Teleport Count non-linear scaling constraints', () => {
    // 1. Lower degrees (half rounded up if < 8)
    ...
    assert.strictEqual(CONFIG.getTeleportCount(5), 3, 'Degree 5 should yield 3 teleports');
    assert.strictEqual(CONFIG.getTeleportCount(6), 3, 'Degree 6 should yield 3 teleports');
    assert.strictEqual(CONFIG.getTeleportCount(7), 4, 'Degree 7 should yield 4 teleports');
    
    // 2. Pivot boundary (exactly 4)
    assert.strictEqual(CONFIG.getTeleportCount(8), 4, 'Degree 8 should yield exactly 4 teleports');
    
    // 3. Accelerating growth (non-linear scaling)
    assert.strictEqual(CONFIG.getTeleportCount(9), 5, 'Degree 9 should yield 5 teleports');
    assert.strictEqual(CONFIG.getTeleportCount(10), 7, 'Degree 10 should yield 7 teleports');
    assert.strictEqual(CONFIG.getTeleportCount(11), 9, 'Degree 11 should yield 9 teleports');
    assert.strictEqual(CONFIG.getTeleportCount(12), 12, 'Degree 12 should yield 12 teleports');
    assert.strictEqual(CONFIG.getTeleportCount(13), 14, 'Degree 13 should yield 14 teleports');
    assert.strictEqual(CONFIG.getTeleportCount(14), 17, 'Degree 14 should yield 17 teleports');
    assert.strictEqual(CONFIG.getTeleportCount(15), 20, 'Degree 15 should yield 20 teleports');
    
    // 4. High degree target (exactly 24)
    assert.strictEqual(CONFIG.getTeleportCount(16), 24, 'Degree 16 should yield exactly 24 teleports');
});
