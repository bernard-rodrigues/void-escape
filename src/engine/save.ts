 ...
 * @returns {string}
 */
function encodeMatrix(arr: Int8Array): string {
 ...
 * @returns {Int8Array}
 */
function decodeMatrix(b64: string): Int8Array {
 ...
 * @returns {object}
 */
function serialiseHunter(h: any): any {
    return {
        x: h.x,
        y: h.y,
        z: h.z,
 ...
 */
export function saveGame(engine: any): void {
    try {
        const snapshot = {
            version: 1,
            timestamp: Date.now(),
            ...
            mapCompletion100Triggered: engine.mapCompletion100Triggered || false,
            dialogueUpTriggered: engine.dialogueUpTriggered || false,
 ...
 */
export function restoreHunter(hunter: any, data: any): void {
    hunter.x = data.x;
    hunter.y = data.y;
    hunter.z = data.z;
 ...
 */
export function restoreMatrix(mazeGen: any, encodedMatrix: string): void {
