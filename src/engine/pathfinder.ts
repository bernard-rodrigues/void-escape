...
class MinHeap {
    ...
    push(item: { f: number, node: Point3D }) {
 ...
 * @returns {number}
 */
export function proximeterDistance(start: Point3D, end: Point3D, maze: Int8Array, size: number, wallType: number = 0, maxDist: number = 10): number {
    if (start.x === end.x && start.y === end.y && start.z === end.z) return 0;
